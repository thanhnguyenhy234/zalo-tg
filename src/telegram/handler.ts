...
import { type AttachmentSource, Reactions, ThreadType } from "zca-js";
...
async function getLocalApiStatus(serverUrl: string): Promise<string> {
	...
	try {
		...
		if (pid) {
			...
			try {
				const { stdout: mem } = await execFileAsync("ps", [
					"-p",
					pid,
					"-o",
					"rss=",
				...
				if (!Number.isNaN(kb))
					lines.push(`💾 RAM: <code>${(kb / 1024).toFixed(1)} MB</code>`);
			} catch {
				/* ignore */
			}
		} else {
	...
	try {
		...
		try {
			...
			await fh.read(
				...
				0,
				buf.length,
				Math.max(0, Number(logStat.size) - buf.length),
			);
	...
	try {
		...
		if (cols.length >= 5) {
			lines.push(
				`💿 Disk /: ${cols[1]} total, ${cols[2]} used, ${cols[3]} avail (<b>${cols[4]}</b>)`,
			);
		}
	} catch {
		/* ignore */
	...
	if (entities) {
		for (const e of entities) {
			if (e.type === "mention") {
				...
				const rawName =
					e.user.first_name + (e.user.last_name ? ` ${e.user.last_name}` : "");
				const uid =
					(zaloId
						? userCache.resolveByNameInGroup(rawName, zaloId)
						: userCache.resolveByName(rawName)) ??
...
function buildReplyAutoMention(
	...
	prefix: string;
	mention: { pos: number; uid: string; len: number };
} | null {
...
function summarizeForumTopicIconChoices(
...
async function resolveForumTopicIconEmojiId(
	input: string,
): Promise<string | null> {
	...
	const stickers =
		(await tgBot.telegram.getForumTopicIconStickers()) as ForumTopicIconSticker[];
	if (/^\d+$/.test(clean)) {
...
async function applyManualDmDisplayName(
	topicId: number,
	zaloId: string,
...
function buildTopicUrl(topicId: number): string {
	...
	const internalChatId = chatId.startsWith("-100")
		? chatId.slice(4)
		: chatId.replace(/^-/, "");
...
function isActiveMuteEntry(entry: ZaloMuteEntry): boolean {
	...
	const now =
		entry.currentTime ?? entry.systemTime ?? Math.floor(Date.now() / 1000);
...
async function buildUnreadDashboard(api: ZaloAPI): Promise<{
	...
	const muteResp = await (api.getMute() as Promise<ZaloMuteResponse>).catch(
		(err) => {
	...
	const pushUnreadMarks = (
		items: ZaloUnreadMark[] | undefined,
		type: 0 | 1,
	) => {
	...
	for (const entry of muteResp?.chatEntries ?? []) {
		if (isActiveMuteEntry(entry))
			mutedKeys.add(unreadConversationKey(String(entry.id), 0));
	}
	for (const entry of muteResp?.groupChatEntries ?? []) {
		if (isActiveMuteEntry(entry))
			mutedKeys.add(unreadConversationKey(String(entry.id), 1));
	}
	...
	for (const entry of merged.values()) {
		...
		const numericCount =
			entry.count && entry.count > 0 ? entry.count : fallbackCount;
		const displayCount =
		...
		const item: UnreadDashboardItem = {
			...
			name:
				topicEntry?.name ??
				(entry.type === 1 ? `Nhóm ${entry.zaloId}` : `Chat ${entry.zaloId}`),
	...
	if (soundOn.length === 0 && soundOff.length === 0) {
		lines.push(
			"✅ Hiện không có topic nào đang có unread signal mà bridge nhìn thấy.",
		);
	} else {
	...
	if (unmappedCount > 0) {
		lines.push(
			"",
			`⚠️ Bỏ qua ${unmappedCount} cuộc trò chuyện unread chưa có topic bridge.`,
		);
	}
	if (hasApproximateCount) {
		lines.push(
			"",
			"ℹ️ Ký hiệu <code>≥1</code> nghĩa là bridge mới biết chắc đang còn unread, nhưng chưa có số đếm mới đủ tin cậy.",
		);
	}
	lines.push(
		"",
		"🧠 Nguồn dữ liệu: unread cache do bridge quan sát khi tin nhắn đi vào + mark unread thủ công của Zalo nếu có.",
	);
	lines.push(
		"⏱ Chat 1:1 sẽ tự ẩn nếu bridge không còn thấy unread signal mới trong khoảng 1 phút; group giữ lâu hơn cho tới khi có clear signal đáng tin cậy.",
	);
	if (muteUnavailable) {
		lines.push(
			"",
			"⚠️ Tạm thời không đọc được trạng thái mute từ Zalo, nên tất cả topic đang được xếp vào nhóm bật thông báo.",
		);
	}
...
async function getAllFriendAliases(
	api: ZaloAPI,
): Promise<Array<{ userId: string; alias: string }>> {
	const items: Array<{ userId: string; alias: string }> = [];
	const pageSize = 200;
	const maxPages = 20;

	for (let page = 1; page <= maxPages; page += 1) {
		const resp = (await api.getAliasList(pageSize, page)) as
			| ZaloAliasListResponse
			| undefined;
...
function formatFriendSearchLabel(friend: {
	userId: string;
	displayName: string;
	alias?: string;
}): string {
...
async function refreshFriendsCache(api: ZaloAPI): Promise<void> {
	const [raw, aliasItems] = await Promise.all([
		api.getAllFriends() as Promise<
			Array<{ userId: string; displayName: string }> | undefined
		>,
...
async function ensureDmTopicForUser(
	user: OpenPhoneSearchUser,
): Promise<EnsuredDmTopic> {
	...
	const displayName = cachedFriend
		...
		: preferredDmTopicName(
				user.uid,
				user.display_name || user.zalo_name || `Zalo ${user.uid}`,
			);
	...
	console.log(
		`[open_phone] Created DM topic "${displayName}" (topicId=${topicId})`,
	);
...
async function notifyZaloNotReady(
	topicId: number,
	reason: string,
): Promise<void> {
	const now = Date.now();
	...
	try {
		...
		const newApi = await triggerQRLogin({
				...
				try {
					await tgBot.telegram.sendPhoto(
						...
						{ source: stream },
						{
							...msgOpts,
							caption:
								"📱 Mở ứng dụng <b>Zalo</b> → Cài đặt → Quét mã QR để đăng nhập.",
							parse_mode: "HTML",
						},
					);
				} finally {
			...
			onDeclined: async () => {
				await tgBot.telegram.sendMessage(
					chatId,
					"❌ Đăng nhập bị từ chối trên điện thoại.",
					msgOpts,
				);
			},
			onSuccess: async () => {
		...
		await tgBot.telegram
			.sendMessage(chatId, `❌ Đăng nhập thất bại: ${String(err)}`, msgOpts)
			.catch(() => undefined);
	} finally {
...
export function setupTelegramHandler(
	...
	tgBot.command("login", async (ctx) => {
		...
		if (!isPrivate && !isFromGroup) {
			console.log(
				`[/login] Bỏ qua từ chat ${ctx.chat.id} (không phải group ${config.telegram.groupId} hoặc DM)`,
			);
			return;
		}
		...
		await handleLoginCommand(ctx.chat.id, threadId, (newApi) => {
			currentApi = newApi;
			void onZaloLogin(newApi).catch((e: unknown) =>
				console.error("[/login] onZaloLogin error:", e),
			);
		});
	});
...
🔗 <a href="${url}">Mở topic</a>`,
	...
	tgBot.command("list_kp", async (ctx) => {
		...
		if (entries.length === 0) {
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
				"📭 Chưa có topic nào trong danh sách khắc phục.",
				{ ...replyOpts, parse_mode: "HTML" },
			);
			return;
		}
		...
		const lines = refreshedEntries.map(
			(entry, index) =>
				`${index + 1}. <a href="${entry.url}">${escapeHtml(entry.name)}</a> <code>(topicId=${entry.topicId})</code>`,
		);
		const chunks: string[] = [];
...
${line}`;
		...
		for (const chunk of chunks) {
			await ctx.telegram.sendMessage(config.telegram.groupId, chunk, {
				...replyOpts,
				parse_mode: "HTML",
			});
		}
	...
	tgBot.command("clear_all_kp", async (ctx) => {
		...
		for (const entry of entries) {
			try {
				...
				console.warn(
					`[KP] Failed to reset topic icon for topicId=${entry.topicId}:`,
					err,
				);
			}
		}
	...
	tgBot.command("clear_kp", async (ctx) => {
		...
		try {
			await ctx.telegram.editForumTopic(config.telegram.groupId, topicId, {
				icon_custom_emoji_id: "5420216386448270341",
			});
			iconUpdated = true;
		} catch (err) {
			...
			if (message.includes("TOPIC_NOT_MODIFIED")) {
				...
				console.warn(
					"[KP] Failed to reset topic icon for topicId=" + topicId + ":",
					err,
				);
			}
		}
	...
	tgBot.command(
		["set_user_icon", "set_user_emoji", "set_icon"],
		async (ctx) => {
			if (ctx.chat.id !== config.telegram.groupId) return;
			...
			const replyTo =
				"reply_to_message" in ctx.message
					? (ctx.message as { reply_to_message?: { message_id: number } })
							.reply_to_message
					: undefined;
			...
			const rawArgs = rawText
				.replace(
					/^\/(?:set_user_icon|set_user_emoji|set_icon)(?:@[A-Za-z0-9_]+)?\s*/i,
					"",
				)
				.trim();
			const parts = rawArgs ? rawArgs.split(/\s+/) : [];

			const isClearWord = (s: string) =>
				["clear", "reset", "remove", "none", "del", "delete"].includes(
					s.toLowerCase(),
				);
			...
			const contextUid =
				quote?.uidFrom || (entry?.type === 0 ? entry.zaloId : undefined);
			...
			if (parts.length === 0) {
				...
				await ctx.telegram.sendMessage(config.telegram.groupId, helpText, {
					...replyOpts,
					parse_mode: "HTML",
				});
				return;
			...
			if (parts.length === 1 && isClearWord(parts[0]!)) {
				...
				await ctx.telegram.sendMessage(config.telegram.groupId, msg, {
					...replyOpts,
					parse_mode: "HTML",
				});
				return;
	...
	tgBot.command("set_topic_icon_emoji", async (ctx) => {
		...
		const rawArg = (ctx.message.text ?? "")
			.replace(/^\/set_topic_icon_emoji(?:@[A-Za-z0-9_]+)?\s*/i, "")
			.trim();
		if (!rawArg || rawArg.toLowerCase() === "list") {
			const stickers =
				(await ctx.telegram.getForumTopicIconStickers()) as ForumTopicIconSticker[];
...
`;
	...
	const handleGroupInfoCommand = async (
		ctx: {
			chat?: { id: number };
			message: { text?: string; message_thread_id?: number };
			telegram: typeof tgBot.telegram;
		},
		forceAll = false,
	) => {
		...
		try {
			...
			for (const member of groupData.currentMems ?? []) {
				...
				if (member.id && name) knownNames.set(member.id, name);
			}
			...
			if (missingUids.length > 0) {
				const appNames = await appGetGroupMembersInfo(missingUids).catch(
					() => null,
				);
			...
			const members = memberUids
				.map((uid) => {
					const aliasName = aliasCache.get(uid)?.trim();
					const profileName = knownNames.get(uid);
					return {
						uid,
						name: aliasName || profileName || uid,
						profileName,
			...
			if (!showAll && members.length > displayLimit) {
				headerLines.push(
					"",
					`ℹ️ Đang hiện ${displayLimit}/${members.length} người. Gõ <code>/group_info all</code> để xem hết.`,
				);
			}

			const lines = visibleMembers.map((member, index) => {
				const suffix =
					member.name === member.uid
						? ` <code>${escapeHtml(member.uid)}</code>`
						: member.isAlias &&
								member.profileName &&
								member.profileName !== member.name
							? ` <i>(${escapeHtml(member.profileName)})</i>`
							: "";
				return `${index + 1}. ${escapeHtml(member.name)}${suffix}`;
			});
			...
			for (const line of lines) {
				...
				if (candidate.length > 3500 && current.trim()) {
			...
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
				`❌ Lỗi lấy thông tin nhóm: <code>${escapeHtml(err instanceof Error ? err.message : String(err))}</code>`,
				{ ...replyOpts, parse_mode: "HTML" },
			);
		}
	};
	...
	tgBot.command("group_infoall", async (ctx) =>
		handleGroupInfoCommand(ctx, true),
	);
	...
	tgBot.command("recall", async (ctx) => {
		...
		if (sent) {
			...
			const zaloThreadType =
				sent.threadType === 1 ? ThreadType.Group : ThreadType.User;
			...
			try {
				...
				console.log(
					`[TG→Zalo] Recall ${recalled} msgIds=[${sent.msgIds}] zaloId=${sent.zaloId}`,
				);
				await ctx.reply(`✅ Đã thu hồi ${recalled} tin nhắn trên Zalo`);
			} catch (err) {
				console.error("[TG→Zalo] Recall error:", err);
				await ctx.reply(
					`❌ Thu hồi thất bại: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
			return;
		}
		...
		if (!quote) {
			await ctx.reply(
				"❌ Không tìm thấy tin nhắn đã gửi (chỉ thu hồi được tin mình gửi từ Telegram hoặc tin từ Zalo đã forward)",
			);
			return;
		}
		...
		try {
			await currentApi.undo(
				{
					msgId: Number(quote.msgId),
					cliMsgId: quote.cliMsgId ? Number(quote.cliMsgId) : 0,
				},
				quote.zaloId,
				zaloThreadType,
			);
			...
			console.log(
				`[TG→Zalo] Recall msgId=${quote.msgId} zaloId=${quote.zaloId} (Zalo->TG)`,
			);
	...
	tgBot.command("search", async (ctx) => {
		...
		if (phoneQuery) {
			try {
				...
				const existingTopicId = store.getTopicByZalo(user.uid, 0);
				const button: { text: string; callback_data: string } =
...
✅ = đã có topic • Nhấn để mở nếu đã map, hoặc tạo nếu chưa có`,
		...
		if (!groupsCache.isFresh()) {
			try {
				const rawGroups = (await currentApi.getAllGroups()) as
				...
				console.error("[/search] getAllGroups failed:", err);
			}
		}
		...
		if (groupResults.length > 0)
			parts.push(`👥 <b>Nhóm</b> (${groupResults.length}):`);
		parts.push(
			"",
			"✅ = đã có topic • Nhấn để mở nếu đã map, hoặc tạo nếu chưa có",
		);
	...
	tgBot.command(["send_contact", "contact", "card", "danhba"], async (ctx) => {
		if (ctx.chat.id !== config.telegram.groupId) return;
		const threadId =
			"message_thread_id" in ctx.message
				? (ctx.message.message_thread_id as number | undefined)
		...
		const query = text
			.replace(
				/^\/(?:send_contact|contact|card|danhba)(?:@[A-Za-z0-9_]+)?\s*/i,
				"",
			)
			.trim();
		...
		// 1. Không tham số hoặc `/send_contact list`
		if (!query || query.toLowerCase() === "list") {
			const contacts = loadContacts();
			if (contacts.length === 0) {
				const sampleToml = `# Tạo file tại: ${contactsFilePath}\n\n[sep_hung]\nname = "Nguyễn Văn Hùng"\nphone = "0988123456"\nnote = "Giám đốc Sở"\n\n[[contacts]]\nname = "Bác Sĩ Nam"\nphone = "0912345678"`;
				await ctx.telegram.sendMessage(
					config.telegram.groupId,
					`📇 <b>Danh bạ Zalo</b>\n\n` +
						`File cấu hình: <code>${escapeHtml(contactsFilePath)}</code>\n` +
						`<i>(Chưa có liên hệ nào hoặc file chưa tồn tại)</i>\n\n` +
						`<b>Mẫu file TOML:</b>\n<pre><code class="language-toml">${escapeHtml(sampleToml)}</code></pre>\n\n` +
						`ℹ️ <i>Sửa file trên máy home/Dropbox, bot sẽ tự cập nhật ngay.</i>`,
					{ ...replyOpts, parse_mode: "HTML" },
				);
				return;
			}
			...
			const lines = contacts.map((c, idx) => {
				const phoneStr = c.phone
					? ` • 📞 <code>${escapeHtml(c.phone)}</code>`
					: "";
				const emailStr = c.email
					? ` • ✉️ <code>${escapeHtml(c.email)}</code>`
					: "";
				const uidStr = c.uid
					? `\n   UID: <code>${escapeHtml(c.uid)}</code>`
					: "";
				const noteStr = c.note ? `\n   <i>(${escapeHtml(c.note)})</i>` : "";
				return `<b>${idx + 1}. ${escapeHtml(c.name)}</b> (<code>${escapeHtml(c.key)}</code>)${phoneStr}${emailStr}${uidStr}${noteStr}`;
			});

			const msgContent =
				`📇 <b>Danh bạ Zalo (${contacts.length} liên hệ)</b>\n` +
				`File: <code>${escapeHtml(contactsFilePath)}</code>\n\n` +
				`${lines.join("\n\n")}\n\n` +
				`💡 <b>Cách gửi thông tin liên hệ:</b>\n` +
				`• Chuyển vào topic DM/Group muốn gửi\n` +
				`• Gõ: <code>/send_contact &lt;key | tên | số điện thoại | UID&gt;</code>\n` +
				`• Ví dụ: <code>/send_contact sep_hung</code> hoặc <code>/send_contact 0988123456</code>`;

			await ctx.telegram.sendMessage(config.telegram.groupId, msgContent, {
				...replyOpts,
				parse_mode: "HTML",
			});
			return;
		...
		// 2. Có tham số query: kiểm tra topic
		if (!threadId) {
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
				"❌ Vui lòng sử dụng lệnh này bên trong topic DM hoặc Group Zalo cần gửi.",
				{ ...replyOpts, parse_mode: "HTML" },
			);
			return;
		}
		...
		if (!entry) {
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
				"❌ Topic này không liên kết với cuộc trò chuyện Zalo nào.",
				{ ...replyOpts, parse_mode: "HTML" },
			);
			return;
		}
		...
		if (!contact) {
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
				`❌ Không tìm thấy liên hệ nào khớp với: <code>${escapeHtml(query)}</code>\n\nGõ <code>/send_contact</code> để xem danh sách có sẵn.`,
				{ ...replyOpts, parse_mode: "HTML" },
			);
			return;
		}

		// Gửi liên hệ sang Zalo
		sentMsgStore.markSending(entry.zaloId);
		try {
			const zaloThreadType =
				entry.type === 1 ? ThreadType.Group : ThreadType.User;

			let sentSuccess = false;
			let zaloMsgId: string | number | undefined;

			if (contact.uid) {
				// Có UID: Thử gửi danh thiếp Zalo
				try {
					const cardRes = (await currentApi.sendCard(
						{
							userId: contact.uid,
						...
						| { msgId?: number | string; message?: { msgId?: number | string } }
						| undefined;

					zaloMsgId = cardRes?.msgId ?? cardRes?.message?.msgId;
					sentSuccess = true;
				} catch (cardErr) {
					console.warn(
						`[TG→Zalo] sendCard failed, falling back to text:`,
						cardErr,
					);
					const fallbackText =
						`📇 Danh bạ: ${contact.name}` +
						(contact.phone ? `\n📞 SĐT: ${contact.phone}` : "") +
						(contact.email ? `\n✉️ Email: ${contact.email}` : "") +
						(contact.note ? `\n📝 Ghi chú: ${contact.note}` : "");
					const textRes = (await currentApi.sendMessage(
						{ msg: fallbackText },
						entry.zaloId,
						zaloThreadType,
					)) as { message?: { msgId?: number | string } } | undefined;
					zaloMsgId = textRes?.message?.msgId;

					await ctx.telegram.sendMessage(
						config.telegram.groupId,
						`⚠️ Không gửi được dạng thẻ danh thiếp (do Zalo hạn chế loại chat), đã tự động gửi tin nhắn văn bản thông tin liên hệ của <b>${escapeHtml(contact.name)}</b>.`,
						{ ...replyOpts, parse_mode: "HTML" },
					);
					sentSuccess = true;
				}
			} else {
				// KHÔNG CÓ UID: Trực tiếp gửi tin nhắn văn bản thông tin liên hệ
				const infoText =
					`📇 Danh bạ: ${contact.name}` +
					(contact.phone ? `\n📞 SĐT: ${contact.phone}` : "") +
					(contact.email ? `\n✉️ Email: ${contact.email}` : "") +
			...
			if (sentSuccess) {
				if (zaloMsgId !== undefined) {
					const numZaloMsgId = Number(zaloMsgId);
					sentMsgStore.save(ctx.message.message_id, {
						msgIds: [!Number.isNaN(numZaloMsgId) ? numZaloMsgId : 0],
						zaloId: entry.zaloId,
						threadType: entry.type,
					});
					const ownUid = String(currentApi.getOwnId?.() ?? "");
					msgStore.save(ctx.message.message_id, [String(zaloMsgId)], {
						msgId: String(zaloMsgId),
						cliMsgId: "",
						uidFrom: ownUid,
						ts: String(Math.floor(Date.now() / 1000)),
						msgType: "webchat",
						content: `📇 [Danh bạ] ${contact.name}${contact.phone ? ` - ${contact.phone}` : ""}${contact.email ? ` - ${contact.email}` : ""}`,
						ttl: 0,
						zaloId: entry.zaloId,
						threadType: entry.type,
					});
				...
				await ctx.telegram.sendMessage(
					config.telegram.groupId,
					`✅ Đã gửi thông tin liên hệ của <b>${escapeHtml(contact.name)}</b> sang Zalo!`,
					{ ...replyOpts, parse_mode: "HTML" },
				);
			}
		} catch (err) {
			console.error("[TG→Zalo] /send_contact error:", err);
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
				`❌ Lỗi gửi thông tin liên hệ: <code>${escapeHtml(err instanceof Error ? err.message : String(err))}</code>`,
				{ ...replyOpts, parse_mode: "HTML" },
			);
		} finally {
			sentMsgStore.unmarkSending(entry.zaloId);
		}
	});

	tgBot.command(["add_contact", "set_contact", "save_contact"], async (ctx) => {
		if (ctx.chat.id !== config.telegram.groupId) return;
		const threadId =
			"message_thread_id" in ctx.message
				? (ctx.message.message_thread_id as number | undefined)
		...
		const cmdArgs = text
			.replace(
				/^\/(?:add_contact|set_contact|save_contact)(?:@[A-Za-z0-9_]+)?\s*/i,
				"",
			)
			.trim();

		let rawContent = cmdArgs;
		...
		const repliedText = repliedMsg
			? ("text" in repliedMsg ? (repliedMsg.text ?? "") : "") ||
				("caption" in repliedMsg ? (repliedMsg.caption ?? "") : "")
			: "";

		if (repliedText) {
			if (cmdArgs) {
				const aliasMatch = cmdArgs.match(/^\[?\s*([a-zA-Z0-9_-]+)\s*\]?$/);
				if (aliasMatch) {
					overrideKey = aliasMatch[1];
				}
			}
			rawContent = repliedText;
		}

		const contactsFilePath = getContactsFilePath();

		// Nếu không có nội dung cả từ lệnh lẫn reply -> hiển thị hướng dẫn chi tiết
		if (!rawContent) {
			const guideText =
				`📝 <b>Hướng dẫn Thêm / Cập nhật Danh bạ Zalo</b>\n\n` +
				`File lưu trữ: <code>${escapeHtml(contactsFilePath)}</code>\n\n` +
				`<b>Cách 1: Sao chép danh thiếp / vCard (Nhiều dòng)</b>\n` +
				`Gõ lệnh kèm thông tin:\n` +
				`<code>/add_contact\n` +
				`Name: C Kiều Anh Syt\n` +
				`Mobile: 098 669 62 86\n` +
				`Home: nguyenkieuanhvp85@gmail.com\n` +
				`Note: Sở Y tế</code>\n\n` +
				`<b>Cách 2: Reply tin nhắn chứa danh thiếp</b>\n` +
				`• Reply tin nhắn thông tin liên hệ\n` +
				`• Gõ: <code>/add_contact</code> (hoặc <code>/add_contact [alias]</code> để đặt key riêng)\n\n` +
				`<b>Cách 3: Cú pháp nhanh 1 dòng</b>\n` +
				`• <code>/add_contact [kieu_anh] C Kiều Anh Syt 0986696286 nguyenkieuanhvp85@gmail.com</code>\n` +
				`• Hoặc tự sinh key: <code>/add_contact C Kiều Anh Syt 0986696286</code>\n\n` +
				`<i>(Hỗ trợ các trường: Name/Tên, Mobile/Phone/SĐT, Home/Email, Note/Ghi chú, UID, Key/Alias)</i>`;

			await ctx.telegram.sendMessage(config.telegram.groupId, guideText, {
				...replyOpts,
				parse_mode: "HTML",
			});
			return;
		...
		// Parse thông tin
		const parsed = parseContactInput(rawContent);
		if (overrideKey) {
			parsed.key = overrideKey;
		}

		if (!parsed.name && !parsed.phone) {
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
				`❌ Không nhận diện được <b>Tên</b> hoặc <b>Số điện thoại</b> trong nội dung.\n\nVui lòng kiểm tra lại định dạng hoặc gõ <code>/add_contact</code> để xem hướng dẫn.`,
				{ ...replyOpts, parse_mode: "HTML" },
			);
			return;
		}
		...
		const key =
			parsed.key ||
			generateContactKey(parsed.name || parsed.phone || "contact");
		const name = parsed.name || key;

		const finalEntry: ContactEntry = {
			key,
			name,
		};
		if (parsed.phone) finalEntry.phone = parsed.phone;
		if (parsed.email) finalEntry.email = parsed.email;
		if (parsed.uid) finalEntry.uid = parsed.uid;
		if (parsed.note) finalEntry.note = parsed.note;

		try {
			const { isNew, entry: saved } = saveOrUpdateContact(finalEntry);

			const details: string[] = [
				`🔑 <b>Key (alias):</b> <code>${escapeHtml(saved.key)}</code>`,
				`👤 <b>Tên:</b> <b>${escapeHtml(saved.name)}</b>`,
			];
			if (saved.phone)
				details.push(`📞 <b>SĐT:</b> <code>${escapeHtml(saved.phone)}</code>`);
			if (saved.email)
				details.push(`✉️ <b>Email:</b> <code>${escapeHtml(saved.email)}</code>`);
			if (saved.uid)
				details.push(`🆔 <b>UID:</b> <code>${escapeHtml(saved.uid)}</code>`);
			if (saved.note)
				details.push(`📝 <b>Ghi chú:</b> <i>${escapeHtml(saved.note)}</i>`);

			const successMsg =
				`✅ <b>Đã ${isNew ? "thêm mới" : "cập nhật"} liên hệ danh bạ thành công!</b>\n\n` +
				`${details.join("\n")}\n\n` +
				`📁 <i>File:</i> <code>${escapeHtml(contactsFilePath)}</code>\n` +
				`💡 <i>Để gửi liên hệ này, vào topic Zalo và gõ:</i>\n` +
				`<code>/send_contact ${escapeHtml(saved.key)}</code>`;

			await ctx.telegram.sendMessage(config.telegram.groupId, successMsg, {
				...replyOpts,
				parse_mode: "HTML",
			});
		} catch (err) {
			console.error("[TG] /add_contact error:", err);
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
				`❌ Lỗi khi lưu danh bạ: <code>${escapeHtml(err instanceof Error ? err.message : String(err))}</code>`,
				{ ...replyOpts, parse_mode: "HTML" },
			);
		}
	});
	...
	tgBot.command("unread", async (ctx) => {
		...
		if (!isFromGroup && !isAllowedPrivate) {
			if (isPrivate) {
				...
				console.log(
					`[/unread] Bỏ qua từ chat ${ctx.chat.id} (không phải group ${config.telegram.groupId} hoặc private allowlist)`,
				);
			}
			return;
		}
		...
		const threadId =
			isFromGroup && "message_thread_id" in ctx.message
				? (ctx.message.message_thread_id as number | undefined)
				: undefined;
		const replyOpts = threadId ? { message_thread_id: threadId } : {};
		...
		try {
			...
			await ctx.telegram.sendMessage(targetChatId, dashboard.text, {
				...replyOpts,
				parse_mode: "HTML",
	...
	tgBot.command("open_phone", async (ctx) => {
		...
		try {
			...
			if (!user?.uid) {
				...
				if (requestId)
					lines.push(`<code>REQUEST_ID=${escapeHtml(requestId)}</code>`);
	...
	tgBot.command("message_search", async (ctx) => {
		...
		const query = (ctx.message.text ?? "")
			.replace(/^\/message_search(?:@[A-Za-z0-9_]+)?\s*/i, "")
			.trim();
		if (!query) {
			await ctx.telegram.sendMessage(
				config.telegram.groupId,
		...
		hits.forEach((hit, index) => {
			const topicId = store.getTopicByZalo(
				hit.quote.zaloId,
				hit.quote.threadType,
			);
			const entry =
				topicId !== undefined ? store.getEntryByTopic(topicId) : undefined;
			const sourceName =
				entry?.name ??
				(hit.quote.threadType === 1
					? `Nhóm ${hit.quote.zaloId}`
					: `Chat ${hit.quote.zaloId}`);
			lines.push(`${index + 1}. ${sourceName}`);
			lines.push(`   ${hit.snippet}`);
			lines.push("");
			buttons.push([
				{
					text: `${index + 1}. ${truncateSearchLabel(sourceName)}`,
					url: buildMessageUrl(hit.tgMsgId),
				},
			]);
		});
		...
		await ctx.telegram.sendMessage(config.telegram.groupId, lines.join("\n"), {
			...replyOpts,
			reply_markup: { inline_keyboard: buttons },
		});
	});
	...
	tgBot.command("addfriend", async (ctx) => {
		...
		try {
			...
			if (status?.is_friend) statusLine = "✅ Đã là bạn bè";
				...
				statusLine = "📩 Họ đang chờ bạn chấp nhận";
	...
	async function getFriendRequestsMessage(page: number) {
		...
		const receivedReqs = (recvRecommends ?? [])
			.filter(
				(item: any) => item.dataInfo?.recommType === 2 || item.recommType === 2,
			)
		...
		if (allItems.length === 0) {
			return {
				text: "✅ Không có lời mời nào đang chờ.",
		...
		const parts: string[] = [
			`📋 <b>Danh sách lời mời (Trang ${safePage + 1}/${totalPages})</b>
`,
		];
		...
		if (pagedRecv.length > 0) {
			...
			for (const { data: u } of pagedRecv) {
				...
				const msg = u.recommInfo?.message
					? ` — "${escapeHtml(u.recommInfo.message)}"`
					: "";
				parts.push(`• ${name}${msg}`);
...
📤 <b>Đã gửi:</b>`);
			for (const { data: u } of pagedSent) {
				const name = escapeHtml(
					u.displayName || u.zaloName || u.userId || "không rõ",
				);
				const msg = u.fReqInfo?.message
					? ` — "${escapeHtml(u.fReqInfo.message)}"`
					: "";
				parts.push(`• ${name}${msg}`);
				inlineKeyboards.push([
					{
						text: `❌ Thu hồi lời mời ${name}`,
						callback_data: `ufr:${u.userId}`,
					},
				]);
			}
		}
...
📬 <b>Nhóm:</b>`);
			for (const { data: inv } of pagedGroup) {
				...
				const exp = new Date(Number(inv.expiredTs) * 1000).toLocaleDateString(
					"vi-VN",
				);
				parts.push(`• 👥 <b>${escapeHtml(g.name)}</b> (${g.totalMember} TV)
  Mời bởi: ${escapeHtml(inv.inviterInfo.dName)} · HH: ${exp}`);
				inlineKeyboards.push([
					{ text: `✅ Tham gia ${g.name}`, callback_data: `jgi:${g.groupId}` },
				]);
			}
		}
		...
		if (safePage > 0) {
			navButtons.push({
				text: "⬅️ Trang trước",
				callback_data: `frq_pg:${safePage - 1}`,
			});
		}
	...
	tgBot.command("joingroup", async (ctx) => {
		...
		try {
			...
			try {
				...
				/* info fetch failure is non-fatal */
			}
			...
			if (errCode === 178 || errMsg.includes("178")) {
				await ctx.telegram.sendMessage(
					config.telegram.groupId,
					"⚠️ Bạn đã là thành viên nhóm này rồi.",
					replyOpts,
				);
			} else if (
				errCode === 240 ||
				errMsg.toLowerCase().includes("waiting") ||
				errMsg.includes("240")
			) {
				await ctx.telegram.sendMessage(
					config.telegram.groupId,
					"⏳ Nhóm yêu cầu duyệt thành viên. Yêu cầu tham gia đã được gửi — chờ admin duyệt.",
					replyOpts,
				);
			} else {
				await ctx.telegram.sendMessage(
					config.telegram.groupId,
					"❌ Không thể tham gia nhóm. Link có thể đã hết hạn hoặc không hợp lệ.",
					replyOpts,
				);
			}
		}
	...
	tgBot.command("status", async (ctx) => {
		...
		if (currentApi) {
			try {
				...
				const name =
					info?.profile?.displayName ?? info?.profile?.zaloName ?? "?";
		...
		if (config.telegram.localServer) {
			const apiDetail = await getLocalApiStatus(
				config.telegram.localServer,
			).catch(() => "❓ Không kiểm tra được");
	...
	tgBot.command("update", async (ctx) => {
		...
		if (!found) {
			await ctx.reply("✅ Bridge đã ở phiên bản mới nhất.", {
				parse_mode: "HTML",
			});
		}
	});
	...
	tgBot.command("admin", async (ctx) => {
		...
		if (parts.length >= 2 && parts[1] === "lookup") {
			...
			if (!reply) {
				await ctx.reply(
					"ℹ️ Reply vào tin nhắn muốn tra mapping rồi gõ /admin lookup",
				);
				return;
			}
			...
			if (sent) {
				...
				lines.push(
					`   Type:   <code>${sent.threadType === 1 ? "Group" : "User"}</code>`,
				);
			} else {
	...
	tgBot.on("callback_query", async (ctx) => {
		...
		if (data?.startsWith("lock_poll:")) {
			...
			try {
				...
				try {
					await ctx.answerCbQuery("❌ Lỗi khoá bình chọn");
				} catch {
					/* ignore */
				}
			}
		...
		if (data?.startsWith("lg:")) {
			...
			try {
				...
				await ctx.telegram
					.closeForumTopic(config.telegram.groupId, topicId)
					.catch(() => undefined);
				await ctx.telegram
					.sendMessage(
						config.telegram.groupId,
		...
		if (data?.startsWith("af:")) {
			...
			try {
				await currentApi.sendFriendRequest(
		...
		if (data?.startsWith("admin:")) {
			...
			if (action === "status") {
				...
				const uptimeBar =
					"█".repeat(Math.min(h, 24)) + "░".repeat(Math.max(0, 24 - h));
			...
			if (action === "cache") {
				...
				const mBar =
					"█".repeat(Math.min(Math.round(mPct / 5), 20)) +
					"░".repeat(Math.max(0, 20 - Math.round(mPct / 5)));
				const text =
			...
			try {
				...
				if (
					...
					try {
						...
						console.log(
							`[sc/sg] Topic ${existing} was closed for ${entityId} — reopened successfully`,
						);
					} catch (reopenErr) {
						console.error(
							`[sc/sg] Topic ${existing} was closed for ${entityId} — reopen failed, keeping mapping:`,
							reopenErr,
						);
					}
					topicAlive = true;
					...
					console.warn(
						`[TopicRecovery] Removing mapping topic=${existing} zalo=${entityId} reason=${checkMsg}`,
					);
		...
		if (!isGroup) {
			...
			if (!displayName) {
				try {
					const resp = (await currentApi?.getUserInfo(entityId)) as
						| {
								changed_profiles?: Record<
									string,
									{ displayName?: string; zaloName?: string }
								>;
								unchanged_profiles?: Record<
									string,
									{ displayName?: string; zaloName?: string }
								>;
						  }
					...
					const realName =
						profile?.displayName?.trim() || profile?.zaloName?.trim();
			...
			displayName = groupsCache
				.search("", Number.MAX_SAFE_INTEGER)
				.find((g) => g.groupId === entityId)?.name;
			if (!displayName) {
				try {
		...
		try {
			...
			console.log(
				`[search/cb] Created ${isGroup ? "group" : "DM"} topic "${displayName}" (topicId=${topicId})`,
			);
	...
	tgBot.on("message_reaction", async (ctx) => {
		try {
			...
			const isEmoji = (r: { type: string }): r is EmojiReaction =>
				r.type === "emoji";
			...
			const added = update.new_reaction
				...
				.filter(
					(r) =>
						isEmoji(r as any) && !oldEmojis.has((r as any).emoji as string),
				);
			...
			if (!zaloIcon) {
				console.log(
					`[TG→Zalo] Reaction: no Zalo map for TG emoji "${tgEmoji}"`,
				);
				return;
			}
			...
			try {
				await currentApi.addReaction(zaloIcon as Reactions, {
			...
			console.log(
				`[TG→Zalo] Reaction "${tgEmoji}" → Zalo "${zaloIcon}" on msg ${quote.msgId}`,
			);
		} catch (err) {
	 ...
	 */
	function getZaloQuote(
		tgMsgId: number | undefined,
	): ZaloQuoteData | undefined {
		...
		if (fromMsgStore) {
			...
			if (!fromMsgStore.cliMsgId || fromMsgStore.cliMsgId === "0") {
				console.log(
					`[TG→Zalo] getZaloQuote: found in msgStore but cliMsgId not yet confirmed (${fromMsgStore.cliMsgId}) — skipping quote for tgMsgId=${tgMsgId}`,
				);
				return undefined;
			}
			console.log(
				`[TG→Zalo] getZaloQuote: found in msgStore for tgMsgId=${tgMsgId} msgId=${fromMsgStore.msgId} cliMsgId=${fromMsgStore.cliMsgId}`,
			);
			return fromMsgStore;
		}
		console.log(
			`[TG→Zalo] getZaloQuote: no quote found for tgMsgId=${tgMsgId}`,
		);
		return undefined;
	}
		...
		try {
			...
			const topicId =
				"message_thread_id" in msg
					? (msg.message_thread_id as number | undefined)
					: undefined;
			if (!topicId) return;
			...
			if (!api) {
				console.warn(
					"[TG→Zalo] currentApi is null – waiting up to 30s for Zalo...",
				);
				api = await waitForZaloApi(() => currentApi, 30_000);
			}
			if (!api) {
				await notifyZaloNotReady(
					topicId,
					"Zalo still not connected after wait – message not forwarded",
				);
				return;
			}
			...
			const notifyError = async (action: string, err: unknown) => {
				...
				console.error(
					`[TG→Zalo] ${action} failed (zaloId=${zaloId}, type=${threadType}):`,
					err,
				);
			...
			if ("text" in msg && msg.text) {
				...
				const pendingNamePrompt =
					pendingNamePromptStore.getPendingNamePrompt(topicId);
				if (
					pendingNamePrompt &&
					replyToMsgId === pendingNamePrompt.promptMsgId
				) {
				...
				console.log(
					`[TG→Zalo] sendMessage → zaloId=${zaloId} type=${threadType} text="${msg.text.slice(0, 80)}"`,
				);
				...
				const _rawTextMentions = resolveTgMentions(
					msg.text,
					("entities" in msg ? msg.entities : undefined) as
						| ReadonlyArray<TgEntity>
						| undefined,
				...
				const _textAutoMention = buildReplyAutoMention(
					replyToMsgId,
					threadType,
				);
				const finalText = _textAutoMention
					? _textAutoMention.prefix + msg.text
					: msg.text;
				const zaloMentions = _textAutoMention
					? [
							_textAutoMention.mention,
							..._rawTextMentions.map((m) => ({
								...m,
								pos: m.pos + _textAutoMention.prefix.length,
							})),
						]
					: _rawTextMentions;
				...
				try {
					...
					for (let ci = 0; ci < chunks.length; ci++) {
						...
						const chunkMentions = zaloMentions
							.filter(
								(m) =>
									m.pos >= chunkOffset &&
									m.pos < chunkOffset + chunkText.length,
							)
					...
					if (zaloMsgId !== undefined) {
						sentMsgStore.save(msg.message_id, {
							msgIds: [zaloMsgId],
							zaloId,
							threadType,
						});
						...
						console.log(
							`[TG→Zalo] msgStore.save for tgMsgId=${msg.message_id} msgId=${zaloMsgId} ownUid=${ownUid}`,
						);
			...
			const TG_FILE_LIMIT = config.telegram.localServer
				...
				: 20 * 1024 * 1024; // 20 MB
			const TG_FILE_LIMIT_LABEL = config.telegram.localServer
				? "2 GB"
				: "20 MB";
			const notifyTooBig = async (filename: string, sizeBytes?: number) => {
				...
				await notifyError(
					...
					new Error(
						`File${sizeMb} vượt giới hạn ${TG_FILE_LIMIT_LABEL} của Telegram Bot API — không thể tải xuống`,
					),
				);
			};
			...
			const sendAttachment = async (
				...
				const replyToMsgId =
					"reply_to_message" in msg
						? (msg as { reply_to_message?: { message_id: number } })
				...
				try {
					...
					if (msg2.includes("file is too big")) {
						await notifyTooBig(filename, fileSize);
						return;
					}
					...
					if (
						config.telegram.localServer &&
						(msg2.includes("wrong file_id") ||
							msg2.includes("temporarily unavailable"))
					) {
						console.warn(
							`[TG→Zalo] Local server can't resolve file_id, falling back to official API: ${filename}`,
						);
						try {
							...
							console.error(
								"[TG→Zalo] Official API fallback failed:",
								fallbackErr,
							);
							await ctx
								.reply(
									`⚠️ Không thể tải file "${filename}". Hãy gửi lại file.`,
				...
				try {
					console.log(
						`[TG→Zalo] Sending ${filename} → zaloId=${zaloId} type=${threadType}`,
					);
					...
					if (
						!["jpg", "jpeg", "png", "webp", "gif", "mp4"].includes(
						...
						api
							.sendMessage(
								{
									...
									...(effectiveCaption.length && zaloQuote
										? { quote: zaloQuote }
										: {}),
									...(captionMentions?.length
							...
							.catch(async (err: unknown) => {
								...
								if ((err as { code?: number })?.code === 114) {
									console.warn(
										"[TG→Zalo] code 114 on attachment+quote, retrying without quote",
									);
					...
					const zaloMsgId =
						sendResult?.message?.msgId ?? sendResult?.attachment?.[0]?.msgId;
					if (zaloMsgId !== undefined) {
						sentMsgStore.save(msg.message_id, {
							msgIds: [zaloMsgId],
							zaloId,
			...
			const _captionReplyMsgId =
				"reply_to_message" in msg
					? (msg as { reply_to_message?: { message_id: number } })
							.reply_to_message?.message_id
					: undefined;
			const _autoMentionForMedia = buildReplyAutoMention(
				_captionReplyMsgId,
				threadType,
			);
			...
			const getCaptionMentions = () => {
				const cap =
					"caption" in msg ? (msg as { caption?: string }).caption : undefined;
				const capEntities =
					"caption_entities" in msg
						? (msg as { caption_entities?: ReadonlyArray<TgEntity> })
								.caption_entities
						: undefined;
				...
				if (_autoMentionForMedia) {
					...
					return {
						cap: cap
							? _autoMentionForMedia.prefix + cap
							: _autoMentionForMedia.prefix.trimEnd(),
						capMentions,
					};
				}
				return {
					cap,
					capMentions: rawMentions.length ? rawMentions : undefined,
				};
			};
			...
			const flushMediaGroup = async (
				...
				try {
					for (const item of items) {
						...
						localPaths.push(
							await downloadToTemp(fileLink.toString(), item.fname),
						);
					...
					try {
						...
						if (sendResult?.message?.msgId != null)
							zaloMsgIds.push(sendResult.message.msgId);
						...
						if (zaloMsgIds.length > 0) {
							...
							for (let i = 0; i < downloadedTgIds.length; i++) {
								...
								const msgIdForItem =
									zaloMsgIds[
										Math.min(attStartIdx + i, zaloMsgIds.length - 1)
									] ??
									zaloMsgIds[0] ??
									"";
								sentMsgStore.save(tgId, {
									msgIds: zaloMsgIds,
									zaloId: meta.zaloId,
							...
							console.log(
								`[TG→Zalo] Media group sent: ${localPaths.length} files, msgIds=[${zaloMsgIds}]`,
							);
						}
					} finally {
			...
			if ("photo" in msg && msg.photo && msg.photo.length > 0) {
				...
				const mediaGroupId =
					"media_group_id" in msg
						? (msg as { media_group_id?: string }).media_group_id
						: undefined;
				if (mediaGroupId) {
					const replyToMsgId = msg.reply_to_message?.message_id;
					mediaGroupStore.add(
			...
			if ("video" in msg && msg.video) {
				const vid = msg.video;
				const fname = vid.file_name?.endsWith(".mp4")
					? vid.file_name
					: `video_${Date.now()}.mp4`;
				const { cap, capMentions } = getCaptionMentions();
				const mediaGroupId =
				...
				let fileLink: URL;
				try {
					fileLink = await ctx.telegram.getFileLink(vid.file_id);
				} catch (err: unknown) {
					const isTooBig =
						err instanceof Error && err.message.includes("file is too big");
					if (isTooBig) {
						await notifyTooBig(fname, vid.file_size);
						return;
					}
				...
				try {
					...
					try {
						localThumbPath = await extractVideoThumbnail(localVideoPath);
					} catch {
						/* no thumb */
					}
					...
					// eslint-disable-next-line @typescript-eslint/no-explicit-any
					const videoUploads: any[] = await api.uploadAttachment(
						[localVideoPath],
						zaloId,
						threadType,
					);
					const videoUpload = videoUploads?.find(
						(r: { fileType?: string }) => r.fileType === "video",
					...
					if (localThumbPath) {
						try {
							// eslint-disable-next-line @typescript-eslint/no-explicit-any
							const thumbUploads: any[] = await api.uploadAttachment(
								[localThumbPath],
								zaloId,
								threadType,
							);
							const tu = thumbUploads?.[0] as
								| { normalUrl?: string }
								| undefined;
							if (tu?.normalUrl) thumbUrl = tu.normalUrl;
						} catch {
							/* keep fallback thumbUrl */
						}
					}
					...
					console.error(
						"[TG→Zalo] sendVideo failed, fallback to attachment:",
						err,
					);
			...
			if ("voice" in msg && msg.voice) {
				...
				let fileLink: URL;
				try {
					fileLink = await ctx.telegram.getFileLink(msg.voice.file_id);
				} catch (err: unknown) {
					const isTooBig =
						err instanceof Error && err.message.includes("file is too big");
					if (isTooBig) {
				...
				const oggPath = await downloadToTemp(
					fileLink.toString(),
					`voice_${Date.now()}.ogg`,
				);
				...
				try {
					...
					const voiceResult = (await api.sendVoice(
						voiceDurationMs > 0
							? { voiceUrl, duration: voiceDurationMs }
							: { voiceUrl },
						zaloId,
						threadType,
					...
					const voiceMsgId =
						voiceResult?.msgId ??
						(voiceResult?.message as Record<string, unknown> | undefined)
							?.msgId;
					if (voiceMsgId != null && !Number.isNaN(Number(voiceMsgId))) {
						sentMsgStore.save(msg.message_id, {
							msgIds: [Number(voiceMsgId)],
							zaloId,
							threadType,
						});
						const ownUid = String(api.getOwnId?.() ?? "");
					...
					console.error(
						"[TG→Zalo] Voice convert/send failed, falling back to file:",
						err,
					);
			...
			if ("sticker" in msg && msg.sticker) {
				...
				if (sticker.is_video) {
					...
					try {
						...
						webmPath = await downloadToTemp(
							fileLink.toString(),
							`sticker_${Date.now()}.webm`,
						);
						gifPath = await convertWebmToGif(webmPath);
						sentMsgStore.markSending(zaloId);
						try {
							const sendResult = await api.sendMessage(
								{ msg: "", attachments: [gifPath] },
								zaloId,
								threadType,
							);
						...
						console.error(
							"[TG→Zalo] sticker webm→gif failed, falling back to thumbnail:",
							err,
						);
						// Fallback: send jpg thumbnail
						const thumbId = sticker.thumbnail?.file_id;
						if (thumbId)
							await sendAttachment(thumbId, `sticker_${Date.now()}.jpg`);
					} finally {
					...
					try {
						const fileLink = await ctx.telegram.getFileLink(sticker.file_id);
						tgsPath = await downloadToTemp(
							fileLink.toString(),
							`sticker_${Date.now()}.tgs`,
						);
						...
						console.error(
							"[TG→Zalo] sticker tgs→gif failed, falling back to thumbnail:",
							err,
						);
						const thumbId = sticker.thumbnail?.file_id;
						if (thumbId)
			...
			if ("poll" in msg && msg.poll) {
				...
				console.log(
					`[TG→Zalo] Received TG poll: id=${tgPoll.id} question="${tgPoll.question}" is_anonymous=${tgPoll.is_anonymous}`,
				);

				if (threadType !== 1) {
					await ctx.reply("❌ Chỉ tạo bình chọn được trong nhóm Zalo.", {
						message_thread_id: topicId,
					});
					return;
				}
				...
				try {
					...
					console.log(
						`[TG→Zalo] Zalo poll created: pollId=${created?.poll_id}`,
					);
					...
					const tgPollUUID =
						(botPollMsg as { poll?: { id?: string } }).poll?.id ?? "";
					console.log(
						`[TG→Zalo] Bot TG poll sent: msgId=${botPollMsg.message_id} uuid=${tgPollUUID}`,
					);

					// 3. Build option list from Zalo response
					const zaloPollOptions =
						created?.options ??
						tgPoll.options.map((o: { text: string }, i: number) => ({
							option_id: i,
							content: o.text,
							votes: 0,
						}));
					...
					const scoreLines = zaloPollOptions.map(
						(o: { content: string }) =>
							`${o.content}\n  ${"░".repeat(10)} 0 phiếu (0%)`,
					);
					...
					const tgScoreMsg = await tgBot.telegram.sendMessage(
						...
						scoreText,
						{
							...
							parse_mode: "HTML",
							reply_parameters: {
								message_id: botPollMsg.message_id,
							...
							reply_markup: {
								inline_keyboard: [
									[
										{
											text: "🔒 Khoá bình chọn",
											callback_data: `lock_poll:${lockPollId}`,
										},
									],
								],
							},
					...
					if (created?.poll_id) {
						pollStore.save({
							...
							tgThreadId: topicId,
							options: zaloPollOptions.map(
								(o: { option_id?: number; content: string }, i: number) => ({
			...
			if ("location" in msg && msg.location) {
				...
				const venue =
					"venue" in msg && msg.venue
						? (msg.venue as { title?: string; address?: string })
						: undefined;
			...
			if ("contact" in msg && msg.contact) {
				...
				if (!cardSent) {
					...
					try {
						const result = (await api.sendMessage(
							{ msg: `👤 ${fullName} — ${contact.phone_number}` },
							zaloId,
							threadType,
						)) as { message?: { msgId?: number } };
						const zaloMsgId = result?.message?.msgId;
	...
	async function doLockPoll(
		entry: import("../store.js").PollEntry,
		api: ZaloAPI,
	): Promise<void> {
		...
		try {
			...
			/* already stopped or no permission */
		}
		// Stop original user poll too (if we have its message_id)
		if (entry.tgOrigPollMsgId) {
			try {
				await tgBot.telegram.stopPoll(
					config.telegram.groupId,
					entry.tgOrigPollMsgId,
				);
			} catch {
				/* no admin rights or already stopped */
			}
		}
		...
		try {
			...
			if (detail?.options) {
				const total = detail.options.reduce(
					(s: number, o: { votes: number }) => s + (o.votes ?? 0),
					0,
				);
				const lines = (
					detail.options as Array<{ content: string; votes: number }>
				).map((o) => {
	...
	tgBot.on("poll_answer", async (ctx) => {
		try {
			...
			console.log(
				`[TG→Zalo] poll_answer: poll_id=${tgPollUUID} option_ids=[${answer.option_ids}]`,
			);
				...
				try {
						...
						: {
								inline_keyboard: [
									[
										{
											text: "🔒 Khoá bình chọn",
											callback_data: `lock_poll:${entry.pollId}`,
										},
									],
								],
							};
					try {
						...
						const newMsg = await tgBot.telegram.sendMessage(
							...
							scoreText,
							{
								message_thread_id: entry.tgThreadId,
								parse_mode: "HTML",
								reply_parameters: {
									message_id: entry.tgPollMsgId,
									allow_sending_without_reply: true,
								},
			...
			console.log(
				`[TG→Zalo] Voted poll ${entry.pollId} options [${optionIds}]`,
			);
