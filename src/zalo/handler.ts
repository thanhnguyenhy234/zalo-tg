...
import { applyZaloMarkupHtml, formatGroupMsgHtml, formatGroupMsg, groupCaption, senderEmoji, topicName, truncate, escapeHtml } from '../utils/format.js';
...
const POPULATE_GROUP_MEMBER_CACHE_RETRYABLE_CODES = new Set([-69, 221]);
const POPULATE_GROUP_MEMBER_CACHE_STARTUP_DELAY_MS = 2_000;
const POPULATE_GROUP_MEMBER_CACHE_BATCH_DELAY_MS = 750;

function sleep(ms: number): Promise<void> {
  return new Promise(resolve => setTimeout(resolve, ms));
}

function getZaloErrorCode(err: unknown): number | undefined {
  const code = (err as { code?: unknown })?.code;
  return typeof code === 'number' ? code : undefined;
}

function isPopulateGroupMemberRetryable(err: unknown): boolean {
  const code = getZaloErrorCode(err);
  return code !== undefined && POPULATE_GROUP_MEMBER_CACHE_RETRYABLE_CODES.has(code);
}

async function retryPopulateGroupMemberCall<T>(label: string, fn: () => Promise<T>, attempts = 4): Promise<T> {
  for (let attempt = 0; attempt < attempts; attempt++) {
    try {
      return await fn();
    } catch (err) {
      if (!isPopulateGroupMemberRetryable(err) || attempt === attempts - 1) throw err;
      const code = getZaloErrorCode(err);
      const waitMs = POPULATE_GROUP_MEMBER_CACHE_STARTUP_DELAY_MS * (attempt + 1);
      console.warn(`[Zalo] ${label} rate-limited for retry (code ${code}), wait ${waitMs} ms`);
      await sleep(waitMs);
    }
  }

  throw new Error(`[Zalo] ${label} retry loop exhausted unexpectedly`);
}
...
async function populateGroupMemberCache(api: ZaloAPI, groupId: string): Promise<void> {
  try {
    ...
    if (!groupData) {
      ...
      const info = await retryPopulateGroupMemberCall(`getGroupInfo(${groupId})`, async () => api.getGroupInfo(groupId)) as {
    ...
    if (missingUids.length > 0) {
      ...
      if (stillMissing.length > 0) {
        ...
        for (let i = 0; i < stillMissing.length; i += BATCH) {
          ...
          try {
            resp = await retryPopulateGroupMemberCall(`getUserInfo(${groupId})`, async () => api.getUserInfo(batch)) as typeof resp;
          } catch (err) {
            if (isPopulateGroupMemberRetryable(err)) {
              console.warn(
                `[Zalo] getUserInfo(${groupId}) stayed rate-limited; keep partial cache ${saved}/${allUids.length} for now`,
              );
              break;
            }
            throw err;
          ...
          if (i + BATCH < stillMissing.length) await sleep(POPULATE_GROUP_MEMBER_CACHE_BATCH_DELAY_MS);
        }
      }
    }
    ...
    if (isPopulateGroupMemberRetryable(err)) {
      console.warn(
        `[Zalo] populateGroupMemberCache skipped for ${groupId} after repeated rate limits (code ${getZaloErrorCode(err)}); ` +
        'will keep existing partial cache and rely on live sender data',
      );
      return;
    }
...
const _memberCacheLoading = new Set<string>();

async function warmGroupMemberCache(api: ZaloAPI, groupId: string): Promise<void> {
  if (_memberCacheLoaded.has(groupId) || _memberCacheLoading.has(groupId)) return;

  _memberCacheLoading.add(groupId);
  try {
    await populateGroupMemberCache(api, groupId);
  } finally {
    _memberCacheLoading.delete(groupId);
...
export async function setupZaloHandler(api: ZaloAPI): Promise<void> {
  ...
  // Stagger calls to avoid hitting startup rate limits while still warming in background.
  const startupGroups = store.all().filter(e => e.type === 1 /* Group */);
  void (async () => {
    for (let i = 0; i < startupGroups.length; i++) {
      if (i > 0) await sleep(POPULATE_GROUP_MEMBER_CACHE_STARTUP_DELAY_MS);
      await warmGroupMemberCache(api, startupGroups[i].zaloId);
    }
  })().catch(err => {
    console.warn('[Zalo] startup member-cache warmup aborted:', err);
  });
  ...
  async function processZaloMessage(msg: ZaloMessage, isRetry = false): Promise<void> {
    // Skip messages sent by the bot (TG→Zalo echo) but NOT messages
      ...
      if (type === 1 && !_memberCacheLoaded.has(zaloId) && !_memberCacheLoading.has(zaloId)) {
        void warmGroupMemberCache(api, zaloId);
      }
      ...
      // Tin gửi từ app Zalo (isSelf, KHÔNG phải bot echo — bot echo đã return ở
      // trên): vẫn mirror lên topic Telegram nhưng gửi silent (disable_notification)
      // để không ping/âm báo. Áp dụng cho mọi loại tin (text/photo/video/...).
      const silentSelf = msg.isSelf;
      const tgBase: {
      ...
      if (silentSelf) tgBase.disable_notification = true;

      const groupSenderUid = type === ThreadType.Group && senderUid ? String(senderUid) : undefined;
      const caption = groupCaption(bridgeSenderName, groupSenderUid);
      ...
      if (msgType === ZALO_MSG_TYPES.TEXT || (text !== null)) {
        ...
        const bodyHtml = (safeMentions?.length || safeStyles?.length)
          ...
          : escapeHtml(safeBody);
        const tgText = formatGroupMsgHtml(bridgeSenderName, bodyHtml, groupSenderUid);
        const sent = await tg.sendMessage(
          config.telegram.groupId,
          tgText,
      ...
      if (msgType === ZALO_MSG_TYPES.PHOTO) {
        ...
        zaloAlbumStore.add(
          ...
          msg.data.msgId,
          zaloMsgIds,
          { senderName: bridgeSenderName, senderUid: groupSenderUid, topicId, tgBase, zaloQuote: zaloQuoteData },
          async (buf) => {
                ...
                const sent = await tg.sendPhoto(
                  ...
                  { source: stream },
                  {
                    ...
                    caption: photoCaption
                      ? `${groupCaption(buf.senderName, buf.senderUid)}
...
${escapeHtml(photoCaption)}`
        ...
        try {
          ...
          try {
            ...
            if (isAnimated) {
              ...
              const animCaption = `${groupCaption(bridgeSenderName, groupSenderUid)} <i>(sticker động 🎥)</i>`;
              const stream = createReadStream(localPath);
              try {
      ...
      if (msgType === ZALO_MSG_TYPES.LINK) {
        ...
        const linkText  = `${groupCaption(bridgeSenderName, groupSenderUid)}\n<a href="${href}">${safeTitle}</a>`;
      ...
      if (msgType === ZALO_MSG_TYPES.WEBCONTENT) {
          ...
          try {
            ...
            if (dataUrl) {
              ...
              if (info) {
                ...
                const fullCaption = `${groupCaption(bridgeSenderName, groupSenderUid)}\n${caption}`;
                const sent = await tg.sendPhoto(
                  config.telegram.groupId,
                  { source: qrBuf },
        ...
        const text = `${groupCaption(bridgeSenderName, groupSenderUid)}\n${body}`;
        ...
        if (lat !== undefined && lng !== undefined) {
          ...
          // Send sender name as a follow-up caption since sendLocation has no HTML caption
            await tg.sendMessage(
              config.telegram.groupId,
              `${groupCaption(bridgeSenderName, groupSenderUid)}📍 Vị trí`,
              { ...tgBase, parse_mode: 'HTML' },
            );
          saveTgMapping(sent);
        } else {
        ...
        if (action === 'create' && !existingEntry) {
          ...
          if (options.length < 2) {
            ...
            const text = type === ThreadType.Group
              ? `${groupCaption(bridgeSenderName, groupSenderUid)}📊 <b>${escapeHtml(question)}</b>\n<i>Cuộc bình chọn mới (${options.length} lựa chọn)</i>`
          ...
          const header = type === ThreadType.Group
            ? `${groupSenderUid ? senderEmoji(groupSenderUid) + ' ' : ''}${bridgeSenderName} tạo bình chọn`
            : 'Bình chọn mới';
          ...
          const header = type === ThreadType.Group
            ? `${groupSenderUid ? senderEmoji(groupSenderUid) + ' ' : ''}${bridgeSenderName} vừa bình chọn`
            : 'Cập nhật bình chọn';
      ...
      // Zalo sends contact cards as msgType 'chat.forward' with contactUid in content
      {
        ...
        if (contactUid || msgType === ZALO_MSG_TYPES.CONTACT) {
          ...
          const fullText = type === ThreadType.Group ? `${groupCaption(bridgeSenderName, groupSenderUid)}\n${body}` : body;
      ...
      if (msgType === ZALO_MSG_TYPES.ECARD) {
        ...
        const lines: string[] = [];
        if (type === ThreadType.Group) lines.push(groupCaption(bridgeSenderName, groupSenderUid));
      ...
      const fallback = type === ThreadType.Group
        ? `${groupCaption(bridgeSenderName, groupSenderUid)}\n<i>[${msgType}]</i>`
        : `<i>[${msgType}]</i>`;
